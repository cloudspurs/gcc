...
class Context(object):
    ...
    def add_all_configs(self):
        ...
        self.add_config(arch='aarch64',
                        ...
                        gcc_cfg=['CFLAGS=-g3 -O0', 'CXXFLAGS=-g3 -O0'])
        ...
        self.add_config(arch='loongarch32',
                        os_name='linux-gnu',
                        variant='ilp32d',
                        gcc_cfg=['--with-abi=ilp32d', '--disable-multilib', '--disable-libgomp', 'CFLAGS=-g3 -O0', 'CXXFLAGS=-g3 -O0'])
        self.add_config(arch='loongarch64',
                        os_name='linux-gnu',
                        variant='lp64d',
                        gcc_cfg=['--with-abi=lp64d', '--disable-multilib', 'CFLAGS=-g3 -O0', 'CXXFLAGS=-g3 -O0'])
        self.add_config(arch='loongarch64',
                        os_name='linux-gnu',
                        variant='lp64s',
        ...
        self.add_config(arch='m68k',
                        os_name='linux-gnu',
                        gcc_cfg=['--disable-multilib', 'CFLAGS=-g3 -Og', 'CXXFLAGS=-g3 -Og'])
        self.add_config(arch='m68k',
                        os_name='linux-gnu',
                        variant='coldfire',
    ...
    def checkout(self, versions):
        ...
        default_versions = {'binutils': 'vcs-mainline',
                            'gcc': 'vcs-mainline',
                            'glibc': 'vcs-mainline',
                            'gmp': '6.2.1',
                            'linux': '6.0',
    ...
    def checkout_vcs(self, component, version, update):
        ...
        if component == 'binutils':
            ...
            git_url = 'https://mirrors.tuna.tsinghua.edu.cn/git/binutils-gdb.git'
            if version == 'mainline':
                git_branch = 'master'
            else:
            ...
            git_url = 'https://mirrors.tuna.tsinghua.edu.cn/git/glibc.git'
            if version == 'mainline':
                git_branch = 'master'
            else:
    ...
    def gcc_checkout(self, branch, update):
        ...
        if os.access(os.path.join(self.component_srcdir('gcc'), '.svn'),
            ...
            update = False
        if not update:
            self.git_checkout('gcc', 'https://mirrors.tuna.tsinghua.edu.cn/git/gcc.git',
                              branch, update)
...
class Config(object):
    ...
    def build_gcc(self, cmdlist, bootstrap):
        ...
        if bootstrap:
            ...
            langs = 'all' if self.ctx.full_gcc else 'c,c++,fortran'
